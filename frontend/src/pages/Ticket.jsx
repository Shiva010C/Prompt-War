...
export default function Ticket() {
  ...
  const content = (
    <div className="pt-24 px-6 max-w-2xl mx-auto space-y-8 pb-24 w-full">
        ...
        >
          <div className="flex flex-col items-center justify-center w-full max-w-sm">
            ...
            <div className="relative p-8 bg-slate-100 rounded-3xl w-full aspect-square flex items-center justify-center">
              ...
              <div className="absolute top-0 left-0 w-12 h-12 border-t-8 border-l-8 border-primary rounded-tl-2xl pointer-events-none"></div>
              <div className="absolute top-0 right-0 w-12 h-12 border-t-8 border-r-8 border-primary rounded-tr-2xl pointer-events-none"></div>
              <div className="absolute bottom-0 left-0 w-12 h-12 border-b-8 border-l-8 border-primary rounded-bl-2xl pointer-events-none"></div>
              <div className="absolute bottom-0 right-0 w-12 h-12 border-b-8 border-r-8 border-primary rounded-br-2xl pointer-events-none"></div>
            </div>
            
            <p className="mt-8 font-headline text-slate-400 text-sm tracking-[0.3em] uppercase font-bold" aria-label="Ticket code is X J F 9 dash 2024">XJF9-2024</p>
            ...
            <button 
                ...
                className="absolute top-8 right-8 w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-800 hover:bg-slate-200 transition-colors"
            >
      ...
      <section className="relative group perspective">
        <div className="bg-white rounded-[2.5rem] p-4 shadow-2xl shadow-blue-500/10 transition-transform duration-500 hover:rotate-x-2">
          ...
          <button 
            ...
            className="w-full bg-white p-8 flex flex-col items-center justify-center border-b-2 border-dashed border-slate-100 relative cursor-pointer group-hover:bg-slate-50 transition-colors"
          >
            {/* Ticket Punch Holes */}
            <div className="absolute -bottom-4 -left-8 w-8 h-8 bg-background rounded-full pointer-events-none"></div>
            <div className="absolute -bottom-4 -right-8 w-8 h-8 bg-background rounded-full pointer-events-none"></div>
            <div className="relative p-4 bg-white rounded-2xl group-hover:scale-105 transition-transform duration-300">
              <img alt="Ticket QR Code" className="w-56 h-56 grayscale contrast-125 pointer-events-none" src="https://lh3.googleusercontent.com/aida-public/AB6AXuD6LhapHJ6NktKKe472WitIWiLt2-Q48zVIFNphAHxHqu_0UIaZHqEXy12qepxKIMHPclHG5GvzsRkybF_jP1gJEtTh4T0-W2IXxblD-MlcZxLPuy7vt5LBvMqzVAibpHtCgmBQVC2JQcBq-bhcquJuRUm-34K2xh_BJ8yUr_Iegws4j1_cMENdhKVWZ6ADTER-MMAScibSeacVqm7asejSFfZd_lytkfIACbXZX3VefWnV72lx4rkp4vF1ACWvtNw8hXkzEwcKM8CW"/>
              {/* Scanning Guide Corners */}
              <div className="absolute top-0 left-0 w-6 h-6 border-t-4 border-l-4 border-primary rounded-tl-lg pointer-events-none"></div>
              <div className="absolute top-0 right-0 w-6 h-6 border-t-4 border-r-4 border-primary rounded-tr-lg pointer-events-none"></div>
              <div className="absolute bottom-0 left-0 w-6 h-6 border-b-4 border-l-4 border-primary rounded-bl-lg pointer-events-none"></div>
              <div className="absolute bottom-0 right-0 w-6 h-6 border-b-4 border-r-4 border-primary rounded-br-lg pointer-events-none"></div>
            ...
            <div className="absolute top-4 right-4 bg-slate-100 text-slate-400 px-2 py-1 rounded-full text-[10px] font-bold uppercase opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                <span className="material-symbols-outlined text-xs" aria-hidden="true">zoom_out_map</span> Enlarge
            </div>
          </button>
          {/* Seat Information - Editorial Layout */}
          <div className="p-8 space-y-8 bg-slate-50 rounded-b-[2rem]">
            <div className="grid grid-cols-3 gap-4" aria-label="Seat details: Section 102, Row G, Seat 04">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-200/60">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center">
                  <span className="material-symbols-outlined text-slate-600" aria-hidden="true">meeting_room</span>
                </div>
                <div>
      ...
      <button 
        onClick={() => navigate('/map')}
        className="w-full text-left bg-surface-container-high rounded-3xl p-6 flex items-center justify-between group cursor-pointer hover:bg-surface-container-highest transition-colors"
      >
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-tertiary-container flex items-center justify-center">
            <span className="material-symbols-outlined text-on-tertiary-container" aria-hidden="true">map</span>
          </div>
          <div>
        ...
        <span className="material-symbols-outlined text-on-surface-variant group-hover:translate-x-1 transition-transform" aria-hidden="true">chevron_right</span>
      </button>
    </div>
  );
